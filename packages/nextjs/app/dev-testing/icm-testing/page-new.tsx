...
export default function ICMTestingPage() {
  ...
  const [step, setStep] = useState(1); // 1: Preparar, 2: Enviado, 3: Entregado
  ...
  // Temporarily use any to avoid type issues while contract types update
  const { data: teleporterContract } = useDeployedContractInfo("MockTeleporterMessenger" as any);
  ...
  const { data: currentNonce } = useScaffoldReadContract({
    contractName: "MockTeleporterMessenger" as any,
